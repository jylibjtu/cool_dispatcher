...
from implements.redis_zset_mq_broker import RedisZsetMQDealer
...
# 具体业务 -> 业务处理handler对象
handler_dict = {
    busi1: Business1Handler(name=busi1, port_list=port_dict[busi1], deal_batch_num=batch_num_dict[busi1],
                            wait_second=wait_dict[busi1], watcher=wait_dict[busi1], mq_dealer=mq_dealer_impl),
    busi2: Business2Handler(name=busi2, port_list=port_dict[busi2], deal_batch_num=batch_num_dict[busi2],
                            wait_second=wait_dict[busi2], watcher=wait_dict[busi2], mq_dealer=mq_dealer_impl),
}
