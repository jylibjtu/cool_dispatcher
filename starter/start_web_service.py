...
def start(target):
    ...
    while True:
        status, output = get_status_output('ps axu | grep web_service.py | grep {} | grep -v grep | grep -v start_web_service | wc -l'.format(target))
        if int(output) == 0:
            time.sleep(1)
            break
        else:
    ...
    for port in port_list:
        command_str = u'nohup python {work_dir}bin/web_service.py start -t {target} -e {env} -p {port} &>{work_dir}logs/start_{target}_web.log &'.format(
            work_dir=work_dir, port=port, env=env, target=target)
...
def halt(target):
    command_str = u"ps axu | grep web_service.py | grep {}".format(target) + " | grep -v grep | grep -v start_web_service | awk '{print $2}' | xargs kill -9"
    print(command_str)
    os.system(command_str)
    while True:
        status, output = get_status_output(u'ps axu | grep web_service.py | grep {} | grep -v grep | grep -v start_web_service | wc -l'.format(target))
        if int(output) == 0:
